...
## Module 13 — Expenses Management

The authoritative source of **non-inventory operating cost** data (rent, utilities, salaries, licenses, maintenance, marketing, …). Supersedes the Module 1 stub `Expense` model and feeds Dashboard's "Total Expenses" KPI + Reports' profit/loss. Financially sensitive — access is limited to `admin` (full + approval), `accountant` (full operational), and `auditor` (read-only); `cashier`/`pharmacist`/`inventory_manager` have no access.

- **Categories** (`/api/expense-categories`, admin-manages) — a pharmacy-customizable list seeded per tenant with 10 sensible defaults (`RENT`, `UTILITIES`, `SALARIES`, `INSURANCE`, `LICENSES_PERMITS`, `EQUIPMENT_MAINTENANCE`, `MARKETING`, `PROFESSIONAL_FEES`, `SUPPLIES_CONSUMABLES`, `MISCELLANEOUS`), schema-ready sub-categories. **Delete-guard**: a category in use is deactivated, not deleted (never orphans financial history).
- **Approval threshold** (Settings `expenses.approval.thresholdAmount`, branch-scoped): an expense at/under it is payable immediately (`NOT_REQUIRED`); a larger one becomes `PENDING_APPROVAL` and **cannot take a payment until an admin approves**. A recurring instance that deviates upward from its template default by more than `expenses.approval.deviationPercent` is flagged too (catches data-entry slips).
- **Payment tracking** — partial payments supported; `paymentStatus` (`UNPAID`/`PARTIALLY_PAID`/`PAID`) mirrors Module 3; `OVERDUE` is computed (unpaid & past due). Payment can't exceed the outstanding balance.
- **Recurring templates** (`/api/recurring-expense-templates`) — `MONTHLY`/`QUARTERLY`/`ANNUALLY` with a `dayOfPeriod` (clamped to a short month's last day). A self-contained daily job generates each period's `Expense` idempotently — a unique `(generatedFromTemplateId, periodKey)` index means a re-run or overlap **never duplicates**. Pause/resume/end without touching historical records. Manual trigger: `POST /run-generation` (also the seam for an external scheduler; set `EXPENSES_GENERATOR_ENABLED=false` to disable the built-in ticker).
- **Consolidated Payables** (`GET /api/expenses/payables/consolidated`) — a **read-only** aggregation merging this module's outstanding expenses with Module 3's outstanding PO payables into one due-date-sorted view, each row tagged by source. Never writes to or duplicates Module 3's data.
- **Summary** (`GET /api/expenses/summary`) — category breakdown for a period; feeds the Dashboard KPI and Reports. Every write is audited (`EXPENSE_CREATED/UPDATED/APPROVED/REJECTED/PAYMENT_RECORDED`, `RECURRING_TEMPLATE_*`, `RECURRING_EXPENSE_AUTO_GENERATED`).

Endpoints: `GET /api/expenses` · `POST /api/expenses` · `GET|PUT /api/expenses/:id` · `POST /api/expenses/:id/approve|reject` · `POST /api/expenses/:id/payments` · `GET /api/expenses/summary` · `GET /api/expenses/payables/consolidated` · CRUD `/api/expense-categories` · `/api/recurring-expense-templates` (+ `/:id/pause|resume|end`, `/run-generation`).

**Env vars:** `EXPENSES_GENERATOR_ENABLED` (default `true`) — set to `false` to turn off the built-in daily recurring-expense ticker (e.g. when driving generation from an external cron against `/run-generation`).

## Module 14 — Reports & Analytics

A **read-only aggregation layer** over Modules 3–13 — the pharmacy's deep, historical, exportable reporting archive (distinct from Module 1's real-time Dashboard). It owns **no** transactional tables; the only data it stores is its own config/export-job rows plus two pre-aggregated read models. Access is **per-report**, mirroring each source module's own role rules (`report-access.ts`) — never a permission bypass. `cashier` has no reporting access.

- **Report set** (`GET /api/reports/*`): Financial — `profit-loss`, `sales-register`, `purchase-summary`, `accounts-payable`, `tax-summary`; Inventory — `stock-valuation`, `stock-movement`, `expiring-stock`, `batch-traceability/:batchId`; Sales & Customer — `top-selling`, `sales-returns`, `customer-history/:customerId`; Supplier & Compliance — `supplier-performance`, `purchase-returns`, `shrinkage`, `controlled-substance-log`, `audit-summary`.
- **Profit & Loss** = net revenue (sales − Module 10 returns) − cost-of-goods (`SaleItem`/`Sale.totalCost`) − Module 13 operating expenses. **Pending-approval expenses are excluded** from the actual net profit (not yet confirmed costs) and shown as a separate supplementary figure. The pure assembler (`pnl.util.ts`) is exhaustively unit-tested so figures reconcile exactly against source data.
- **Pre-aggregation strategy (the performance core).** A nightly job (`daily-summary-aggregation.job.ts`) upserts `DailySalesSummary` (revenue/cost/tax/count/returns per branch/day) and `DailyExpenseSummary` (actual expenses per branch/day/category). Reports read these summaries for older, fully-aggregated days and fall back to **live** queries against Module 4/13 tables for the recent, not-yet-aggregated tail. The split is disjoint and exhaustive (`splitAggregatedVsLive`), so a hybrid result equals a pure-live computation of the same range.
  - **Idempotent + catch-up (spec §21).** Every day is written via `upsert` on the `(pharmacyId, branchId, date[, categoryId])` unique key, recomputed from live data — a re-run never double-counts. Each run re-aggregates a **trailing 35-day window** up to yesterday, so a night the job didn't run is automatically backfilled on the next successful run (no silent hole). Summaries are always regenerable and never authoritative if they drift — live data wins.
- **Export** (`POST /api/reports/export` → `GET /api/reports/export/:jobId`): **async** job model — request returns a `jobId`, generation runs in the background, the frontend polls until `READY`/`FAILED` and auto-downloads. CSV is generated natively; **PDF** uses a dependency-free text-layout writer (`pdf-exporter.service.ts`) — the polished, presentation-grade P&L is the on-screen `ProfitLossStatementView`, printable to PDF via the browser. Every export is audited (`REPORT_EXPORTED`). Max ranges are Settings-driven (`reports.maxSyncRangeDays`, `reports.maxExportRangeDays`).
- **Saved configurations** (`CRUD /api/reports/saved-configurations`) — save a report + filters (incl. **rolling** windows like "last full month" that recompute on each run). `SAVED_REPORT_CONFIGURATION_CREATED/DELETED` audited.

**Env vars:** `REPORTS_AGGREGATION_ENABLED` (default `true`) — set to `false` to turn off the built-in nightly aggregation ticker (e.g. when driving it from an external cron). Frontend: Finance → **Reports**.
